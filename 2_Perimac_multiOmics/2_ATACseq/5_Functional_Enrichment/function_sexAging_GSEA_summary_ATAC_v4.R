...
summarize_SexAge <- function(my.m.gset, my.f.gset, my.gset.name, my.thrs = 0.05){
  ...
  pdf(paste0(Sys.Date(), "_GSEA_enrichment_", my.gset.name, "_GSEA_enrichment_PeriMac_Sex_Aging_FDR5_scatterplot_ATAC.pdf"), width = 6, height = 6)
  ...
  write.table(my.merged.gset.both, file = paste0(Sys.Date(), "_GSEA_enrichment_", my.gset.name, "_GSEA_enrichment_PeriMac_Sex_Aging_FDR", 100* my.thrs, "_Summary_ATAC.txt"),
              sep = "\t", quote = F, row.names = F)
  
  if (sum(my.merged.gset.both$AGREEMENT == "DIVERGENT") > 2) {
    #### Top divergent pathways
    path.disagree    <- my.merged.gset.both[my.merged.gset.both$AGREEMENT == "DIVERGENT"   ,]
    # top 5 strongest enrichment/FDR in F/M that disagree
    ...
    my.top.disagree  <- rbind(top.div.m[1:min(nrow(top.div.m),6), ] , # 6 or as many satisfying criteria
                              top.div.f[1:min(nrow(top.div.f),6), ] ) # 6 or as many satisfying criteria
    ...
    my.max.char <- max(nchar(DIS.gsea.tab$Pathways))
    ...
    pdf( paste(Sys.Date(),"GSEA_aging_BALLOON_plot",my.gset.name,"top_6_DIVERGENT_pathways_PeriMac_Sex_Aging_ATAC.pdf", sep="_"), onefile=T, height = 4, width=5)
    print(my.plot)
    dev.off()
  }
  ...
  # create and preserve wanted display order
  my.max.char <- max(nchar(AGR.gsea.tab$Pathways))
  ...
  my.plot <- my.plot + ggtitle("PeriMac Aging") + labs(x = "-log10(pvalue)", y = "Sex CONVERGENT")
  my.plot <- my.plot + scale_colour_gradientn(colours = my.color.vector.age, na.value = "grey50", guide = "colourbar", values = my.scaled, limits = c(-4, 4))
  my.plot <- my.plot + scale_size_area(limits = c(0,30)) + scale_y_discrete(labels = wrap_format(40))
  my.plot
  
  pdf( paste(Sys.Date(),"GSEA_aging_BALLOON_plot",my.gset.name,"top_10_CONVERGENT_pathways_PeriMac_Sex_Aging_ATAC.pdf", sep="_"), onefile=T, height = 4, width=5)
  print(my.plot)
  dev.off()
