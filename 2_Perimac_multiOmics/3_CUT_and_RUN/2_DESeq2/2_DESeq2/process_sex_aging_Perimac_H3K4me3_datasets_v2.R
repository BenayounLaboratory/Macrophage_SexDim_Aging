setwd('/Volumes/BB_Home_HQ/Immune_sex_dimorphism_Aging/Macrophages/Clean_Macrophage_Data/CUT_and_RUN/H3K4me3_Aging/H3K4me3_height/DEseq2_H3K4me3_height')
...
# 2021-09-29
# process H3K4me3 CUT&RUN dataset
...
my.mph1 <- read.csv('../Diffbind/2021-09-30_PeriMac_H3K4me3_Sex_Aging_Normalized_count_matrix.txt', sep = "\t", header = T)
...
my.peak.annot <- read.csv('../Diffbind/HOMER_2021-09-30_PeriMac_H3K4me3_Sex_Aging_diffbind_peaks.xls', sep = "\t", header = T)
...
my.filtered.matrix <- my.mph[my.good,-1] # 47594 peaks
...
my.Age <- c(rep(4,9),rep(20,8))
my.Sex <- c(rep("F",4),rep("M",5),rep("F",3),rep("M",5))
...
my.outprefix <- paste(Sys.Date(),"PeriMac_H3K4me3_height_DESeq2_Analysis",sep="_")
...
plot(x,y,
     ...
     cex.lab = 1.5,
     cex.axis = 1.5,
     main="H3K4me3 height (PCA)")
dev.off()


# expression range
pdf(paste(my.outprefix,"_Normalized_H3K4me3_height_boxplot.pdf"))
boxplot(tissue.cts,col=my.colors,cex=0.5,ylab="Log2 DESeq2 Normalized counts", las = 2, outline = F)
dev.off()

# plot X and Y H3K4me3 peak depths
pdf(paste(my.outprefix,"_Normalized_counts_Xist_H3K4me3_height_barplot.pdf"))
...
pdf(paste(my.outprefix,"_Normalized_counts_Y_chromosome_H3K4me3_height_barplot.pdf"))
...
my.num.aging <- length(genes.aging) # 2254

my.heatmap.out <- paste(my.outprefix,"AGING_Heatmap_FDR5.pdf", sep = "_")
pdf(my.heatmap.out, onefile = F)
my.heatmap.title <- paste("Aging significant (FDR<5%), ", my.num.aging, " H3K4me3 peaks (height)",sep="")
...
save(res.age, file = paste(Sys.Date(),"PeriMac_H3K4me3_height_Aging_BOTH.RData", sep ="_"))
...
my.num.sex <- length(genes.sex) # 1499

my.heatmap.out <- paste(my.outprefix,"SEX_DIM_Heatmap_FDR5.pdf", sep = "_")
pdf(my.heatmap.out, onefile = F)
my.heatmap.title <- paste("Sex significant (FDR<5%), ", my.num.sex, " H3K4me3 peaks (height)",sep="")
...
save(res.sex, file = paste(Sys.Date(),"PeriMac_H3K4me3_height_SEX.RData", sep ="_"))
...
my.outprefix.age_M <- paste(Sys.Date(),"PeriMac_H3K4me3_height_DESeq2_model_with_AGING_Males",sep="_")
my.outprefix.age_F <- paste(Sys.Date(),"PeriMac_H3K4me3_height_DESeq2_model_with_AGING_Females",sep="_")
...
pdf(paste(Sys.Date(),"PeriMac_H3K4me3_height_aging_male_vs_female_FC_scatterplot_FDR5_10.pdf", sep = "_"))
...
write.table(my.merged.FM[my.F_M.10,], file = paste(Sys.Date(),"PeriMac_H3K4me3_height_AGING_SexAgreement_FDR10_genes_statistics.txt", sep ="_"), sep = "\t" , row.names = T, quote=F)
...
pdf(paste(Sys.Date(),"PeriMac_H3K4me3_height_aging_male_vs_female_FC_scatterplot_divergent_FDR5.pdf", sep = "_"))
smoothScatter(my.merged.FM$log2FoldChange_F,my.merged.FM$log2FoldChange_M, 
...
write.table(my.merged.FM[my.M_notF,], file = paste(Sys.Date(),"PeriMac_H3K4me3_height_AGING_Male.NOT.Female_FDR5_genes_statistics.txt", sep ="_"), sep = "\t" , row.names = T, quote=F)
write.table(my.merged.FM[my.F_notM,], file = paste(Sys.Date(),"PeriMac_H3K4me3_height_AGING_Female.NOT.Male_FDR5_genes_statistics.txt", sep ="_"), sep = "\t" , row.names = T, quote=F)

write.table(my.merged.FM, file = paste(Sys.Date(), "PeriMac_H3K4me3_height_AGING_Separated_Sex_Merged_Table_ALL_genes_statistics.txt", sep ="_"), sep = "\t" , row.names = T, quote=F)
write.table(res.age.F, file = paste(Sys.Date()   , "PeriMac_H3K4me3_height_AGING_Female_ALL_genes_statistics.txt", sep ="_"), sep = "\t" , row.names = T, quote=F)
write.table(res.age.M, file = paste(Sys.Date()   , "PeriMac_H3K4me3_height_AGING_Male_ALL_genes_statistics.txt", sep ="_"), sep = "\t" , row.names = T, quote=F)
...
pdf(my.heatmap.out, onefile = F)
my.heatmap.title <- paste("Female bias ", length(my.F_notM.peak), " H3K4me3 peaks (height)",sep="")
