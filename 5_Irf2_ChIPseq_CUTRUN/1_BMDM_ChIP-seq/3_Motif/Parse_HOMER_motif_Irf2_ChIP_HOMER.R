setwd('/Users/berenice/Desktop/Irf2_CUTRUN/ChIPseq/HOMER_Motifs')
options(stringsAsFactors = F)

# 2025-12-01
# parse/plot Irf2 ChIPseq motif enrichment data
# run with HOMER peaks to match cut and run pipeline

############################################
# 1. Read in HOMER motif enrichment in sex-specifc changes with age ATAC peaks
Irf2.cr.motifs  <- read.csv("2025-11-30_HOMER_MSPC_Irf2_BMDM_ConsensusPeaks//knownResults.txt" , sep = "\t", header = T)
...
sig.Irf2.cr.motifs  <- Irf2.cr.motifs  [Irf2.cr.motifs  $q.value < 0.05,]  # 44

write.table(sig.Irf2.cr.motifs, file = paste0(Sys.Date(),"_HOMER_Motifs_Enriched_in_Irf2_ChIP_FDR5.txt"), quote = F, sep = "\t", row.names = F)
...
pdf(paste0(Sys.Date(),"HOMER_Motifs_Enriched_Top5_Irf2_BMDM_ChIP.pdf"), height = 5, width=8)
