...
##################################################################
# 1. Read RAW results
cts.irf2     <- read.table('2024-08-16_RAW_sh_Irf2_data_DESeq2_log2_counts_matrix.txt', sep = "\t", header = T)
deg.irf2     <- read.table('2024-08-16_RAW_sh_Irf2_data_DESeq2_shIrf2_DEseq2_results_COMBINED_FDR1e-4_genes_statistics.txt', sep = "\t", header = T)
...
pdf(paste0(Sys.Date(),"_shIrf2_RAW267.4_Normalized_counts_boxplot_Hk3.pdf"), height = 5, width = 3)
boxplot(Hk.exp, outline = F, las = 1, ylim = c(12,15) , main = "Hk3 (RAW267.4)", ylab = "DEseq2 VST normalized log2 counts", cex.axis = 0.75)
beeswarm(Hk.exp, add = T, pch = 16, pwcol = my.colors)
