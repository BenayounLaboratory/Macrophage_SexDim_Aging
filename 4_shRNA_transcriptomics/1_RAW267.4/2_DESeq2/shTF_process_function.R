...
process_shTF <- function (my.raw.data, my.TF, shNon = 1:3, shLuc = 4:6, shTF1 = 7:9, shTF2 = 10:12) {
  ...
  colnames(my.raw.data) <- gsub("_RNAseq_SET2_2_STAR_Aligned.sortedByCoord.out.bam","",   colnames(my.raw.data))
  colnames(my.raw.data) <- gsub("_RNAseq_SET2_STAR_Aligned.sortedByCoord.out.bam","",   colnames(my.raw.data))
  colnames(my.raw.data) <- gsub("_RNAseq_SET3_STAR_Aligned.sortedByCoord.out.bam","",   colnames(my.raw.data))
  colnames(my.raw.data) <- gsub("_RNAseq_SET3_1_STAR_Aligned.sortedByCoord.out.bam","",   colnames(my.raw.data))
  colnames(my.raw.data) <- gsub("_RNAseq_SET4_STARAligned.sortedByCoord.out.bam","",   colnames(my.raw.data))
  ...
  my.matrix3   <- my.raw.data[my.good.data,-1] # 15216 genes for Irf2 example
  
  # get output file prefixes
  my.outprefix <- paste(Sys.Date(),"RAW_sh", my.TF ,"data_DESeq2",sep="_")
  ...
  legend("topleft",c("shNT","shLuc",paste0("sh",my.TF,"_1"),paste0("sh",my.TF,"_2")), col = c("orange","peru","seagreen","seagreen3"), pch = 16, cex = 0.75, bty = 'n')
  dev.off()
  
  #### Meis1 has too low expression to be detected by this RNAseq (too shallow)
  ...
  if (my.TF != "Meis1") {
    ...
    beeswarm(TF.exp, add = T, pch = 16, pwcol = my.colors)
    dev.off()
  }
  ...
  genes.shTF1 <- rownames(res.shTF1)[res.shTF1$padj < 0.05] # 3142 for Irf2
  genes.shTF2 <- rownames(res.shTF2)[res.shTF2$padj < 0.05] # 4401 for Irf2
  ...
  my.num.TF <- length(genes.TF) # 2057 genes for Irf2
