setwd('/Volumes/BB_HQ_1/Immune_sex_dimorphism_Aging/Macrophages/Clean_Macrophage_Data/Peritoneal_scRNAseq/v2_set/Muscat_PB')
...
# 2025-08-28
# Run Muscat DE on all QC cell types
...
DefaultAssay(my.perit.singlets.consistent) <- "RNA"

#### Make covariate table
perit.meta <- unique(my.perit.singlets.consistent@meta.data[,c("Sample_ID", "Condition")])
rownames(perit.meta) <- perit.meta$Sample_ID
write.table(perit.meta, file = paste0(Sys.Date(),"_NIAv2_sample_metadata_table.txt"), sep = "\t", row.names = F, quote = F)

# convert to SingleCellExperiment
# https://satijalab.org/seurat/archive/v3.1/conversion_vignette.html
my.perit.singlets.consistent.sce <- as.SingleCellExperiment(my.perit.singlets.consistent)
save(my.perit.singlets.consistent.sce, file = paste(Sys.Date(),"SingleCellExperimnents_peritoneal_NIAv2.RData",sep = "_"))

rm(my.perit.singlets.consistent) # free up some memory
###############################################################################################
...
#######        Data preparation         #######
###############################################
perit.sce.cl <- prepSCE(my.perit.singlets.consistent.sce, 
...
# OFCohort1    2693         269        23        2     355
# OFCohort2    1620         267        14        3     215
# OMCohort1    2224         702        36        5     571
# OMCohort2    1047         354        15        1     246
# YFCohort1    2060        1645        40        9     125
# YFCohort2    1002         582        44       12     137
# YMCohort1     576         395        12        1      62
# YMCohort2     644         721        24        6      46
...
#### save counts
save(counts.pb.perit, perit.celltype.qc, file = paste0(Sys.Date(),"_muscat_PB_perit_NIAv2_QC_Clean.RData"))
##############################################################################################
...
# perit.meta     <- read.table('2025-08-28_NIAv2_sample_metadata_table.txt', header = T)
...
my.rdata.age <- paste0(Sys.Date(),"_pseudobulk_NIAv2_DEseq2_objects.RData")
save(deseq.res.list.perit, deseq.res.list.perit.F, deseq.res.list.perit.M, file = my.rdata.age)

my.rdata.age.2 <- paste0(Sys.Date(),"_pseudobulk_NIAv2_MergedFM_DEseq2_objects.RData")
save(deseq.res.list.perit.FM, file = my.rdata.age.2)

my.vst.age <- paste0(Sys.Date(),"_pseudobulk_NIAv2_VST_data_objects.RData")
save(vst.cts.perit, file = my.vst.age)
################################################################################################


#######################
sink(file = paste(Sys.Date(),"_scRNAseq_peritoneal_NIAv2_session_Info.txt", sep =""))
sessionInfo()
sink()
