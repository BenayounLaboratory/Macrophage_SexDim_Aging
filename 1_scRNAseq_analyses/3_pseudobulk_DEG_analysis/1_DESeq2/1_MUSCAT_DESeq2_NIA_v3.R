setwd('/Volumes/BB_HQ_1/Immune_sex_dimorphism_Aging/Macrophages/Clean_Macrophage_Data/Peritoneal_scRNAseq/v3_NIA/Muscat_PB')
...
# 2025-08-28
# Run Muscat DE on all QC cell types (NIA v3 set)
...
DefaultAssay(NIA.singlets.consistent) <- "RNA"

#### Make covariate table
perit.meta <- unique(NIA.singlets.consistent@meta.data[,c("Sample_ID", "Condition")])
rownames(perit.meta) <- perit.meta$Sample_ID
write.table(perit.meta, file = paste0(Sys.Date(),"_NIAv3_sample_metadata_table.txt"), sep = "\t", row.names = F, quote = F)

# convert to SingleCellExperiment
# https://satijalab.org/seurat/archive/v3.1/conversion_vignette.html
NIA.singlets.consistent.sce <- as.SingleCellExperiment(NIA.singlets.consistent)
save(NIA.singlets.consistent.sce, file = paste(Sys.Date(),"SingleCellExperimnents_peritoneal_NIAv3.RData",sep = "_"))

rm(NIA.singlets.consistent) # free up some memory
###############################################################################################
...
#######        Data preparation         #######
###############################################
perit.sce.cl <- prepSCE(NIA.singlets.consistent.sce, 
...
# NIA_OF1     397         193         3        2      24
# NIA_OF2     851         377         2        0      63
# NIA_OM1     317         507         6        0     179
# NIA_OM2     657         568        11        1      80
# NIA_YF1     267         382         5        6      10
# NIA_YF2     634         922        23        5      36
# NIA_YF3     417         510        15        1      37
# NIA_YM1     364        1192        14        6      25
# NIA_YM2     369         514        14        9      73
# NIA_YM3     225         408         7        5      25
...
#### save counts
save(counts.pb.perit, perit.celltype.qc, file = paste0(Sys.Date(),"_muscat_PB_perit_NIAv3_QC_Clean.RData"))
##############################################################################################
...
# perit.meta     <- read.table('2025-08-28_NIAv3_sample_metadata_table.txt', header = T)
...
my.rdata.age <- paste0(Sys.Date(),"_pseudobulk_NIAv3_DEseq2_objects.RData")
save(deseq.res.list.perit, deseq.res.list.perit.F, deseq.res.list.perit.M, file = my.rdata.age)

my.rdata.age.2 <- paste0(Sys.Date(),"_pseudobulk_NIAv3_MergedFM_DEseq2_objects.RData")
save(deseq.res.list.perit.FM, file = my.rdata.age.2)

my.vst.age <- paste0(Sys.Date(),"_pseudobulk_NIAv3_VST_data_objects.RData")
save(vst.cts.perit, file = my.vst.age)
################################################################################################


#######################
sink(file = paste(Sys.Date(),"_scRNAseq_peritoneal_NIAv3_session_Info.txt", sep =""))
sessionInfo()
sink()
